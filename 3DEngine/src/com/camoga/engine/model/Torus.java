...
public class Torus extends Model {
	 ...
	 */
	public Torus(double R, double r, int longNum, int shortNum, double scale, Sprite sprite) {
		super(vertices(longNum, shortNum, R, r), faces(longNum, shortNum), quadTexCoords(longNum,shortNum), scale, sprite);
	}
	...
	private static double[][] vertices(int longNum, int shortNum, double R, double r) {
		...
		for(int i = 0; i < shortNum; i++) {
			...
			for(int j = 0; j < longNum; j++) {
				vert[j+i*longNum] = new double[] {Ro*Math.sin(2*Math.PI*j/longNum),r*Math.cos(2*Math.PI*i/shortNum),-Ro*Math.cos(2*Math.PI*j/longNum)};
			}
		}
		return vert;
	}
