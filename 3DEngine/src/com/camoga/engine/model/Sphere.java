...
//		if(latitude%2!=0) throw new RuntimeException("latitude can't be odd");
	...
	private static int[][] faces(int longitude, int latitude) {
		...
		for(int j = 0; j < longitude; j++) {
			fac[j] = new int[] {0,(j+1)%longitude+1,j%longitude+1};
		}
		...
		for(int j = 0; j < longitude; j++) {
			fac[j+(latitude-1)*longitude] = new int[] {longitude*(latitude-1)+1,j%longitude+1+longitude*(latitude-2),(j+1)%longitude+1+longitude*(latitude-2)};
		}
